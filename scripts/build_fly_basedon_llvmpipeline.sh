...
# ---------------------------------------------------------------------------
BUILD_DIR="${FLY_BUILD_DIR:-${REPO_ROOT}/build-llvmpipeline-fly}"
if [[ "${BUILD_DIR}" != /* ]]; then
...
if [ -z "${MLIR_PATH:-}" ]; then
  candidates=(
    "${BASE_DIR}/llvm-pipeline-flydsl/build-flydsl/mlir_install"
    "${BASE_DIR}/llvm-pipeline/build-flydsl/mlir_install"
    "${BASE_DIR}/llvm-pipeline/mlir_install"
  )
