...
# ---------------------------------------------------------------------------
BUILD_DIR="${FLY_BUILD_DIR:-${REPO_ROOT}/build-fly-based-zanllvm}"
if [[ "${BUILD_DIR}" != /* ]]; then
...
if [ -z "${MLIR_PATH:-}" ]; then
  candidates=(
    "${BASE_DIR}/llvm-zan-flydsl/build-flydsl/mlir_install"
    "${BASE_DIR}/llvm-zan/build-zan-llvm/mlir_install"
    "${BASE_DIR}/llvm-zan/mlir_install"
  )
