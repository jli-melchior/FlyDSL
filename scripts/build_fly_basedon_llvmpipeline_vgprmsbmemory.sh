...
# ---------------------------------------------------------------------------
BUILD_DIR="${FLY_BUILD_DIR:-${REPO_ROOT}/build-fly-llvmpipeline-vgpr-vmem}"
if [[ "${BUILD_DIR}" != /* ]]; then
...
if [ -z "${MLIR_PATH:-}" ]; then
  candidates=(
    "${BASE_DIR}/llvm-pipeline/build-vgpr-vmem-flydsl/mlir_install"
    "${BASE_DIR}/llvm-pipeline/mlir_install_vpgr_vmem"
  )
